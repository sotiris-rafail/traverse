		...
		for (Field field : fields) { 
			...
			if (field.get(objectToTest) instanceof List) {
				...
				if (newList != null && publishedList != null) {
						...
						for(int i=0; i< max; i++) {
							if (publishedList.get(i) != null && newList.get(i) != null) { // Check for difference
									if(!publishedList.get(i).equals(newList.get(i))) {
										diffs.add(new Diffs(publishedList.get(i), newList.get(i), depth, field.getName()));
									}
							} else if (publishedList.get(i) != null) {// value has been deleted
								diffs.add(new Diffs(publishedList.get(i), "HAS BEEN DELETED", depth, field.getName()));
							} else if (newList.get(i) != null) { // New value exists 
								diffs.add(new Diffs("DID NOT EXIST", newList.get(i), depth, field.getName()));
							}
						}
					...
					newList.forEach(item -> diffs.add(new Diffs("DID NOT EXIST", item, finalDepth, field.getName())));
				} else if (publishedList != null) {
					System.out.println("new is NULL");
					final int finalDepth = depth;
					publishedList.forEach(item -> diffs.add(new Diffs(item, "HAS BEEN DELETED", finalDepth, field.getName())));
				} else {
					// DO NOTHING
				}	
			} else {
				...
				if (existingValue != null) {
					if (!isPrimitive(existingValue.getClass().getName())) {
						...
						if(!existingValue.equals(newValue)) {
							diffs.add(new Diffs(existingValue, newValue, depth, field.getName()));
						}
					} else {// value has been deleted
						diffs.add(new Diffs(existingValue, "HAS BEEN DELETED", depth, field.getName()));
					} 
				} else if (newValue != null) { // New value exists 
					if (isPrimitive(newValue.getClass().getName())) {
						diffs.add(new Diffs("DID NOT EXIST", newValue, depth, field.getName()));
					} else  { // Call self
						depth++;
